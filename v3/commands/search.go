...
func newSearchPlatformCmd(env *CommandEnv) *cobra.Command {
...
func newSearchLibCmd(env *CommandEnv) *cobra.Command {
	searchCmd := &cobra.Command{
		Use:     "libraries",
...
func newSearchCmd(env *CommandEnv) *cobra.Command {
	searchCmd := &cobra.Command{
		Use:   "search",
		Short: "Search for arduino platforms, libraries, and boards",
		Long:  "\nSearch for arduino platforms, libraries, and boards",
	}
	searchCmd.AddCommand(newSearchPlatformCmd(env))
	searchCmd.AddCommand(newSearchLibCmd(env))
	return searchCmd
}
